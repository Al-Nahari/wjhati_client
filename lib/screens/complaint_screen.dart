...
class _ChatListScreenState extends State<ChatListScreen> {
  final String chatsApiUrl = 'http://192.168.1.9:8000/chats';
  List<dynamic> chats = [];
  ...
  Future<int> getUnreadCountForChat(int chatId) async {
    try {
      final response = await http.get(Uri.parse('http://192.168.1.9:8000/messages/'));
...
class _ChatDetailScreenState extends State<ChatDetailScreen> {
  ...
  void initState() {
    super.initState();
    messagesApiUrl = 'http://192.168.1.9:8000/messages/';
    fetchMessages();
  }
      ...
      if (message['is_read'] == false && message['sender'] != currentUserId) {
        try {
          final response = await http.patch(
            Uri.parse(
                'http://192.168.1.9:8000/messages/${message['id']}/'),
            headers: {
